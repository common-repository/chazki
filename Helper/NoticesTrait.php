...
trait NoticesTrait {
				...
				public static function check_notices() {
								...
								foreach ($notices_types as $type) {
												$notices = get_transient('chazki-' . $type . '-notices');
												if (empty($notices)) {
																continue;
												}
												foreach ($notices as $notice) {
																echo '<div class="'. esc_attr( 'notice notice-' . $type . ' is-dismissible)') .'">';
																echo '<p>' . esc_html($notice) . '</p>';
																echo '</div>';
												}
												delete_transient('chazki-' . $type . '-notices');
								}
				}
				...
				private static function add_notice(string $type, string $msg, bool $do_action = false) {
								...
								set_transient('chazki-' . $type . '-notices', $notices, 60);
								if ($do_action) {
